...
#define g_lpszAplicationTitle TEXT("The main window of the application. Programmer Burlyaev D.A.")


#define MAX_LOADSTRING 100
#define MAX_THREAD_NUMBER 4
#define MAX_STRINGS_NUMBER 2
#define MAXCH 1000
...
hSemaphore = CreateSemaphore(NULL, g_cMax, g_cMax, TEXT("Semaphore"));
	...
	{
		MessageBeep(MB_ICONASTERISK);
		MessageBox(NULL, TEXT("Window not created!"), TEXT("Error"), MB_OK | MB_ICONERROR);
		return FALSE;
	}
...
void WndProc_OnCommand(HWND hWnd, int id, HWND hwndCtl, UINT codeNotify)
{
	...
	case ID_THREAD1_CREATETHREAD:
		...
		if (exitCodeThread != STILL_ACTIVE)
		{
			...
			MessageBox(NULL, TEXT("Thread 1 has already been created"), TEXT(""), MB_OK);
		break;
	...
	case ID_THREAD2_CREATETHREAD:
		...
		if (exitCodeThread != STILL_ACTIVE)
		{
			...
			MessageBox(NULL, TEXT("Thread 2 has already been created"), TEXT(""), MB_OK);
		break;
	...
	case ID_THREAD1_PAUSETHREAD:
		...
		if (exitCodeThread == STILL_ACTIVE & Suspend[1] == false)
		{
			...
			MessageBox(NULL, TEXT("Thread 1 has not yet been created or is already stopped"), TEXT(""), MB_OK);
		break;
	...
	case ID_THREAD2_PAUSETHREAD:
		...
		if (exitCodeThread == STILL_ACTIVE & Suspend[2] == false)
		{
			...
			MessageBox(NULL, TEXT("Thread 2 has not yet been created"), TEXT(""), MB_OK);
		break;
	...
	case ID_THREAD1_STARTTHREAD:
		...
		if (exitCodeThread == STILL_ACTIVE & Suspend[1] == true)
		{
			...
			MessageBox(NULL, TEXT("Thread 1 has not yet been created or has not stopped yet"), TEXT(""), MB_OK);
		break;
	...
	case ID_THREAD2_STARTTHREAD:
		...
		if (exitCodeThread == STILL_ACTIVE & Suspend[2] == true)
		{
			...
			MessageBox(NULL, TEXT("Thread 2 has not yet been created or has not stopped yet"), TEXT(""), MB_OK);
		break;
	...
	case ID_THREAD1_DESTROY:
		...
		if (exitCodeThread == STILL_ACTIVE)
		{
			...
			MessageBox(NULL, TEXT("Thread 1 termination has begun"), TEXT(""), MB_OK);
		}
		else
			MessageBox(NULL, TEXT("Thread 1 has not been created yet"), TEXT(""), MB_OK);
		break;
	...
	case ID_THREAD2_DESTROY:
		...
		if (exitCodeThread == STILL_ACTIVE)
		{
			...
			MessageBox(NULL, TEXT("Stream 2 termination has started"), TEXT(""), MB_OK);
		}
		else
			MessageBox(NULL, TEXT("Stream 2 has not been created yet"), TEXT(""), MB_OK);
		break;
	...
	case ID_THREAD2_PRIORITYUP:
		...
		if (exitCodeThread == STILL_ACTIVE)
		{
			...
			MessageBox(NULL, TEXT("Thread 2 has not been created yet"), TEXT(""), MB_OK);
		break;
	...
	case ID_SYNHRONIZATION_ON:
	{
		...
		EnableMenuItem(GetMenu(hWnd), ID_SYNHRONIZATION_ON, MFS_GRAYED);
		break;
	}
	case ID_SYNHRONIZATION_OFF:
	{
		fContinue = FALSE;
		EnableMenuItem(GetMenu(hWnd), ID_SYNHRONIZATION_OFF, MFS_ENABLED);
		break;
	}
...
#define MAXCHAR 500
	...
	wsprintf(CreepingLine,
		TEXT("Secondary thread was created by D.A. Burlyaev, thread - %d"),
...
BOOL DlgProc_OnInitDialog(HWND hdlg, HWND hwndFocus, LPARAM lParam)
{
	...
	if (Suspend[lParam] == true)
		wsprintf(Buff1, TEXT("State - Waiting"));
	else
		switch (exitCodeThread)
		{
		case STILL_ACTIVE:
			wsprintf(Buff1, TEXT("Status - Active"));
			break;
		case 0:
			wsprintf(Buff1, TEXT("Status - Process not created"));
			break;

		default:
			wsprintf(Buff1, TEXT("Completed with code %d"), exitCodeThread);
		}
	...
	if (GetThreadTimes(hSecThread[lParam], &ftCreation, &ftExit, &ftKernel, &ftUser))
	{
		...
		wsprintf(Buff1, TEXT("%d min, %d sec, %d m.sec"), stWorkind.wMinute, stWorkind.wSecond, stWorkind.wMilliseconds);//����� ����� ��������
