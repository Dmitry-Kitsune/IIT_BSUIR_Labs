...
// Message handler for about box1.
INT_PTR CALLBACK MyDlgproc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    UNREFERENCED_PARAMETER(lParam);
    switch (message)
    ...
    case WM_INITDIALOG:
        {
        TCHAR test[300] = { TEXT("Пример инициализации диалогового окна") };
        SYSTEMTIME st; GetLocalTime(&st);
        TCHAR test1[200];
        wsprintf(test1, TEXT(" Время %d ч %d мин. %d сек."),st.wHour, st.wMinute, st.wSecond);
        lstrcat(test, test1);
        SetDlgItemText(hDlg, IDC_ABOUTTEXT, test);
        } 
        return (INT_PTR)TRUE;
