...
BOOL OnCREATE(HWND hWnd, LPCREATESTRUCT lpCreateStruct)
{
	...
	TextOut(hDCMeta, 100, 100, TEXT("Гедройц Максим Васильевич"), lstrlen(TEXT("Гедройц Максим Васильевич")));

	HPEN hPen = CreatePen(PS_SOLID, 2, RGB(0, 255, 0));
	HBRUSH hBrush = CreateHatchBrush(HS_BDIAGONAL, RGB(255, 0, 0));
	SelectObject(hDCMeta, hPen);
	SelectPen(hDCMeta, hPen);
	Ellipse(hDCMeta, 245, 190, 305, 250);
	SelectBrush(hDCMeta, hBrush);
	Rectangle(hDCMeta, 250, 250, 300, 300);
	DeleteObject(hBrush);
	DeleteObject(hPen);
